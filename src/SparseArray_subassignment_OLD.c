 ...
 *           C_subassign_SVT_by_Lindex_OLD() will get passed a _long_ linear
 *           index.
 *
 ...
 * _long_ incoming data e.g. when C_subassign_SVT_by_Lindex_OLD() is called
...
static SEXP subassign_leaf_by_Lindex_OLD(SEXP leaf, int dim0,
					 SEXP Lindex, SEXP vals)
{
	R_xlen_t nvals = XLENGTH(vals);
	if (nvals > INT_MAX)
...
/****************************************************************************
 * === OLD === C_subassign_SVT_by_[M|L]index() === OLD ===
 */
...
/* --- .Call ENTRY POINT --- */
SEXP C_subassign_SVT_by_Mindex_OLD(SEXP x_dim, SEXP x_type, SEXP x_SVT,
		SEXP Mindex, SEXP vals)
{
	SEXPTYPE Rtype = _get_Rtype_from_Rstring(x_type);
	if (Rtype == 0)
		error("SparseArray internal error in "
		      "C_subassign_SVT_by_Mindex_OLD():\n"
	...
	if (x_ndim == 1)
		return subassign_leaf_by_Lindex_OLD(x_SVT, INTEGER(x_dim)[0],
						    Mindex, vals);
...
/* --- .Call ENTRY POINT --- */
SEXP C_subassign_SVT_by_Lindex_OLD(SEXP x_dim, SEXP x_type, SEXP x_SVT,
		SEXP Lindex, SEXP vals)
{
	SEXPTYPE Rtype = _get_Rtype_from_Rstring(x_type);
	if (Rtype == 0)
		error("SparseArray internal error in "
		      "C_subassign_SVT_by_Lindex_OLD():\n"
		      "    SVT_SparseArray object has invalid type");
	if (TYPEOF(vals) != Rtype)
		error("SparseArray internal error in "
		      "C_subassign_SVT_by_Lindex_OLD():\n"
